...
const PERMISSION_ALIASES = {'RELATORIOS:Gerencial':['RELATORIOS:Visualizar'],'RELATORIOS:Agenda':['RELATORIOS:Visualizar'],'RELATORIOS:Logs':['RELATORIOS:Visualizar'],'RELATORIOS:Limpar Logs':['RELATORIOS:Exportar'],'GESTAO:Unidades':['GESTAO:Visualizar'],'GESTAO:Locais de Realizacao':['GESTAO:Visualizar'],'GESTAO:Professores':['GESTAO:Visualizar'],'GESTAO:Atividades/Oficinas':['GESTAO:Visualizar'],'GESTAO:Turmas/Oficinas':['GESTAO:Visualizar'],'INSCRICOES:Inscricao Alunos da Rede':['INSCRICOES:Visualizar'],'INSCRICOES:Inscricao Comunidade':['INSCRICOES:Visualizar'],'FREQUENCIA:Gerar Folha':['FREQUENCIA:Visualizar']};
const RESOURCE_PERMISSIONS = {units:'GESTAO:Unidades',locations:'GESTAO:Locais de Realizacao',teachers:'GESTAO:Professores',activities:'GESTAO:Atividades/Oficinas',classes:'GESTAO:Turmas/Oficinas',users:'ACESSO:Visualizar'};
...
function catalogUnitId(db, resource, row) {
  if (!row) return 0;
  if (['locations','classes','users','teachers','activities'].includes(resource)) return inferredCatalogUnitId(db, resource, row);
  return 0;
}

function prepareScopedCatalogData(user, resource, data) {
  const row = { ...(data || {}) };
  if (isUnitScoped(user) && ['locations','classes','users','teachers','activities'].includes(resource)) row.unit_id = userUnitId(user);
  return row;
}
...
async function request(method, path, data = undefined) {
  ...
  if (p[0] === 'catalogos') {
    ...
    if (method === 'POST') {
      ...
      if (['locations','classes','users','teachers','activities'].includes(resource)) ensureUnitScope(db, user, data?.unit_id);
    ...
    if (method === 'PUT' && p[2]) {
      ...
      data = prepareScopedCatalogData(user, resource, data);
      if (['locations','classes','users','teachers','activities'].includes(resource)) ensureUnitScope(db, user, data?.unit_id ?? db[resource][idx].unit_id);
    ...
    if (method === 'DELETE' && p[2]) {
      ...
      if (target && ['locations','classes','users','teachers','activities'].includes(resource)) ensureUnitScope(db, user, catalogUnitId(db, resource, target));
