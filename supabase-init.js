...
function hasPermission(user, permission) {
  return isAdminUser(user) || userPermissions(user).includes(permission);
}
...
async function request(method, path, data = undefined) {
  ...
  if (p[0] === 'catalogos') {
    ...
    if (method === 'GET') {
      ...
      else requireAnyPermission(user, ['GESTAO:Visualizar', 'INSCRICOES:Visualizar', 'FREQUENCIA:Visualizar', 'RELATORIOS:Visualizar', 'ACESSO:Visualizar']);
      const rows = scopedCatalogRows(db, user, resource);
      return { rows, total: rows.length };
    }
    if (method === 'POST') {
      requirePermission(user, module + ':Cadastrar');
    ...
    if (method === 'PUT' && p[2]) {
      requirePermission(user, module + ':Editar');
    ...
    if (method === 'DELETE' && p[2]) {
      requirePermission(user, module + ':Excluir');
  ...
  if (p[0] === 'relatorios') {
    ...
    if (p[1] === 'logs' && method === 'GET') { if (!isAdminUser(user)) throw { status: 403, error: 'Apenas o perfil Administrador pode visualizar logs.' }; requirePermission(user, 'RELATORIOS:Visualizar'); return db.logs.slice(0, 200); }
    if (p[1] === 'logs' && method === 'DELETE') {
      if (!isAdminUser(user)) throw { status: 403, error: 'Apenas o perfil Administrador pode limpar logs.' };
      requireAnyPermission(user, ['ACESSO:Excluir', 'RELATORIOS:Exportar']);
    ...
    if (p[1] === 'gerencial') {
      requirePermission(user, 'RELATORIOS:Visualizar');
