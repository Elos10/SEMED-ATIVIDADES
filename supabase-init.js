...
async function request(method, path, data = undefined) {
  ...
  if (p[0] === 'relatorios') {
    if (p[1] === 'logs' && method === 'GET') { requirePermission(user, 'RELATORIOS:Visualizar'); return isUnitScoped(user) ? db.logs.filter((l) => Number(l.user_id || 0) === Number(user.id)).slice(0, 200) : db.logs.slice(0, 200); }
    ...
    if (p[1] === 'gerencial') {
      ...
      return scopedClasses(db, user).map((c) => {
